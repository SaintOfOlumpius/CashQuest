...
class ProfileActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun loadIncomeExpenseData(userId: String) {
        ...
        db.collection("users").document(userId).collection("accounts").get()
            .addOnSuccessListener { accountDocs ->
                val allTransactions = mutableListOf<vc.prog3c.poe.data.models.Transaction>()
                    ...
                    db.collection("users").document(userId)
                        ...
                        .addOnSuccessListener { transactionDocs ->
                            transactionDocs.forEach { transactionDoc ->
                                val transaction = transactionDoc.toObject(vc.prog3c.poe.data.models.Transaction::class.java)
    ...
    private fun loadCategoryBudgetData(userId: String) {
        ...
        db.collection("users").document(userId).collection("categories").get()
            .addOnSuccessListener { categoryDocs ->
                val categories = categoryDocs.mapNotNull { it.toObject(vc.prog3c.poe.data.models.Category::class.java) }
                updateCategoryBudgetChart(categories)
            }
    }

    private fun updateIncomeExpenseChart(transactions: List<vc.prog3c.poe.data.models.Transaction>) {
        ...
        monthlyData.entries.sortedBy { it.key }.forEachIndexed { index, (month, monthTransactions) ->
            val income = monthTransactions.filter { it.type == vc.prog3c.poe.data.models.TransactionType.INCOME }.sumOf { it.amount }
            val expense = monthTransactions.filter { it.type == vc.prog3c.poe.data.models.TransactionType.EXPENSE }.sumOf { it.amount }
    ...
    private fun updateCategoryBudgetChart(categories: List<vc.prog3c.poe.data.models.Category>) {
